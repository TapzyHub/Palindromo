...
# Algoritmo Manacher
def manacher(text):
    # Preproceso del texto para manejar tanto palíndromos de longitud par como impar
    T = '#'.join(f'^{text}$')  # Añade separadores entre caracteres para manejar longitud par e impar
    n = len(T)
    P = [0] * n  # Almacena el radio de los palíndromos
    ...
    # Encontrar el palíndromo más grande en el texto original
    max_len, center_index = max((n, i) for i, n in enumerate(P))
    start = (center_index - max_len) // 2  # Obtener la posición en el texto original
